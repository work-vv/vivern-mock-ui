...
const Request = ({requestId}: RequestProps) => {
  ...
  return <>
    <p>Request</p>
    <form>
      <Input inputName="route" inputValue={route}
             inputPlaceholder="Add route"
             handleInputChange={handleInputChange}/>
      <Select selectName="method" selectValue={method} selectOptions={HttpRequestMethods}
              handleSelectChange={handleInputChange}/>
    </form>
    <p>Parses</p>
    <Button title="+" handleButtonClick={handleAddParse} />
    {parses.map(parseId => < Parse  key={parseId} parseId={parseId}/>)}
  </>
}
